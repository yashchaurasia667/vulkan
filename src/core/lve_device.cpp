...
void LveDevice::createLogicalDevice() {
  ...
  if (!supportedFeatures13.dynamicRendering ||
    ...
    throw std::runtime_error("A required vulkan feature is not supported");
  }
...
bool LveDevice::isDeviceSuitable(VkPhysicalDevice device) {
  ...
  bool isDiscrete = VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU == devProps.deviceType;

  return indices.isComplete() && extensionsSupported && swapChainAdequate &&
         isDiscrete && supportedFeatures.samplerAnisotropy;
}
